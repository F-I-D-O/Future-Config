...
def load_config_data(*config_source_definitions: ConfigSource, use_builder_directives=False):
	...
	for config_source_definition in config_source_definitions:
		...
		if config_source_definition.path:
			config_map_from_source = _change_config_context(config_map_from_source, config_source_definition.path)
...
def _change_config_context(config_map_from_source: ConfigDataObject, path: list):
